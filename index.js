...
client.on("ready", () => {
  ...
  client.user.setActivity(`to ${prefix}help`, {
    type: 'LISTENING'
  }, 'online')
});
...
client.on("message", async (message) => {
  ...
  if (command.guildOnly && message.channel.type === 'dm') {
    ...
    dmOnly.setFooter('Thanks for using Cindy!', message.author.displayAvatarURL({
      dynamic: true
    }))
  ...
  if (command.ownerOnly && message.author.id !== ownerId || coownerId) {
    ...
    devOnly.setFooter('Thanks for using Cindy!', message.author.displayAvatarURL({
      dynamic: true
    }))
  ...
  if (timestamps.has(message.author.id)) {
    ...
    if (now < expirationTime) {
      ...
      cembed.setFooter(message.member.displayName, message.author.displayAvatarURL({
        dynamic: true
      }))
      cembed.setTimestamp()
  ...
  try {
    ...
    eembed.setFooter(message.member.displayName, message.author.displayAvatarURL({
      dynamic: true
    }))
    eembed.setTimestamp()
